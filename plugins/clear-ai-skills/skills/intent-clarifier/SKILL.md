...
확인해야 할 사항이 여러 방식으로 나뉘면 각 항목에 선택지를 함께 제시한다.

- 확인해야 할 각 질문에는 `1.`, `2.`, `3.`처럼 번호를 매긴다.
- 선택지는 사용자가 고르기 쉽게 짧고 서로 구분되게 쓴다.
- 사용자의 조건이나 일반적인 작업 목적에서 더 적합한 선택을 판단할 근거가 있으면 해당 항목 뒤에 `(추천)`을 붙인다.
- 추천하는 이유를 한 문장으로 덧붙인다. 근거가 부족하면 임의로 추천하지 않는다.
- 권장 형식은 다음과 같다.
...
선택지를 제시하더라도 사용자가 아직 고르지 않은 내용을 정리된 요청에 확정해서 넣지 않는다.
